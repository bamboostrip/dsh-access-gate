 ...
 */
function renderLoginPage(next, errorText, preference = "system") {
...
export async function apply(ctx, config = {}) {
    ...
    ctx.inject(["settings"], (sctx) => {
        ...
        themePreference = () => {
            const preference = sctx.settings.get("ui-theme")?.preference;
    ...
    /** 当前生效的免密网段（仅对非回环来源生效）。 */
    const trustedPrefixes = () => current().trustedRemotePrefixes ?? [];
    /** 当前生效的登录有效期。 */
    const tokenTtl = () => current().tokenTtlMs ?? DEFAULT_TTL_MS;
    /**
