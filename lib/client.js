 ...
 *    （settings.plugin.item keyed slot，v0.6.0 适配 rc.7：list→keyed），
 *    配置/清除访问密码：
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * settings.plugin.item 的分发 key：host 侧 settings 服务注册的
		 * namespace（src/index.ts 的 SETTINGS_NAMESPACE），不是 locale 命名空间。
		 */
		var SETTINGS_NS_KEY = "dsh-access-gate";
		...
		// 2) 访问认证设置卡片（官方"插件配置"选项卡的 keyed slot）。
		// rc.7 起 settings.plugin.item 是 keyed slot：按"host 侧 settings 服务
		// 注册的 namespace"分发（dsh-client-ui-settings-plugins 的
		// ConfigurablePluginsTabController 只渲染两边交集 —— host serve 的
		// namespace × client 注册的卡片 key），key 必须与 host 半边
		// src/index.ts 的 SETTINGS_NAMESPACE 一致，否则卡片静默不出现。
